...
// Field represents a column definition for tabular outputs.
type Field struct {
	Key    string
	Header string
}

// FieldSelection captures the active columns along with flags that control
// which optional values (comment/message/URL/PRs) are needed from the engine.
...
// ResolveFields interprets CLI flags into a concrete column selection.
...
// Headers returns the header labels for the provided fields.
func Headers(fields []Field) []string {
...
// RowValues extracts the field values from an item according to the given fields.
func RowValues(it engine.Item, fields []Field) []string {
	row := make([]string, len(fields))
	for i, f := range fields {
		row[i] = FormatFieldValue(it, f.Key)
	}
	return row
}

// FormatFieldValue converts an engine.Item attribute to its string representation.
